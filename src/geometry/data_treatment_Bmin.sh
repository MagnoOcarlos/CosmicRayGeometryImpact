...
do
	...
	do	
		...
		do
			...
			#histogram Title	
			sed -i -e "s/Main Title/ Proton: #theta=${j}#circ, B=${k} nG, L_{B}=${l}Mpc, #phi=${1}#circ/g" Fixed_angle_energy_spectrum_Bmin.cc
			...
			g++ Fixed_angle_energy_spectrum_Bmin.cc -o plot.exe `root-config --cflags --glibs`
			...
			do
			 	...
			 	do
					...
					sed -i -e "s/Main Title/ Proton: Angle ${j}#circ, B=${k} nG, L_{B}=${l}Mpc,#phi = ${1}#circ, #gamma=$gamma, Rcut=$Rcut/g"\
					 Fixed_angle_energy_spectrum_Rcut_Bmin.cc
					
					g++ Fixed_angle_energy_spectrum_Rcut_Bmin.cc -o plot.exe `root-config --cflags --glibs`
