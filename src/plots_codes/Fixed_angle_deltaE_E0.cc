...
TH1D* fillingHist( ifstream& f){
	...
	double E=0;
	int rows = count(f);
	TH1D *h = new TH1D("","Main Title", 50, -9,0);
...
void saveBins(TH1D *h20, TH1D *h40,TH1D *h60,TH1D *h80,char *argv[]){//Function to sabe bin content in a different directory
	...
	ofstream outputfile(std::string(argv[2])+"energy_loss_bins_"+std::string(argv[3])+".txt");
	int j;
...
//	TRint *rint = new TRint("", &argc, argv);	
	...
	const char *filename20  ="../delta_20Mpc.txt";
	const char* filename40 ="../delta_40Mpc.txt";
	const char* filename60 ="../delta_60Mpc.txt";
	const char* filename80 ="../delta_80Mpc.txt";
	...
	hArrive20->GetXaxis()->SetTitle("log #frac{ #Delta E}{E_{0}}");
	hArrive20->Draw("HIST");
	...
	auto leg = new TLegend(0.1,0.70,0.48,0.9);
