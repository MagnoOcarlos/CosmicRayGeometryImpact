...
TH1D* fillingHist( ifstream& f){
	...
	double D=0;
	int rows = count(f);
	TH1D *h = new TH1D("","Main Title", 40, 20,4020);
...
void saveBins(TH1D *h20, TH1D *h40,TH1D *h60,TH1D *h80,char *argv[]){//Function to sabe bin content in a different directory
	...
	ofstream outputfile(std::string(argv[2])+"distance_bins_"+std::string(argv[3])+".txt");
	int j;
...
//	TRint *rint = new TRint("", &argc, argv);	
	...
	const char *filename20  ="../Distance_20Mpc.txt";
	const char* filename40 ="../Distance_40Mpc.txt";
	const char* filename60 ="../Distance_60Mpc.txt";
	const char* filename80 ="../Distance_80Mpc.txt";
	...
	hArrive20->GetXaxis()->SetTitle("Distance (Mpc)");
	hArrive20->Draw("HIST");
